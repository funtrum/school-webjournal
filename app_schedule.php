...
$week_id = $_POST['week'];
$day = $_POST['day'];
$func_id = $_POST['func']; //1 - prev, 0 - next --ADDED 2 FOR APP
$multiply = 7;
$res = $db->query("SELECT `users` . *  FROM `users` WHERE `login`='$login'");
    ...
    if($row['password']==$password) {
        ...
        switch ($func_id) {
            case 0:
                ...
                $startDate = date("d.m", strtotime('-'.date("w").' day'.' +1 day'.' +'.$multiply*$week_id.' day'));
                $finishDate = date("d.m", strtotime('-'.date("w").' day'.' +6 day'.' +'.$multiply*$week_id.' day'));
            break;
            case 1:
                $d_date[0] = date("d.m", strtotime('-'.date("w").' day'.' -'.$multiply*$week_id.' day'));
                $d_date[1] = date("d.m", strtotime('-'.date("w").' day'.' +1 day'.' -'.$multiply*$week_id.' day'));
            ...
            break;
            case 2:
                $d_date[0] = date("Y-m-d", strtotime('-'.date("w").' day'));
                $d_date[1] = date("Y-m-d", strtotime('-'.date("w").' day'.' +1 day'));
                $d_date[2] = date("Y-m-d", strtotime('-'.date("w").' day'.' +2 day'));
                $d_date[3] = date("Y-m-d", strtotime('-'.date("w").' day'.' +3 day'));
                $d_date[4] = date("Y-m-d", strtotime('-'.date("w").' day'.' +4 day'));
                $d_date[5] = date("Y-m-d", strtotime('-'.date("w").' day'.' +5 day'));
                $d_date[6] = date("Y-m-d", strtotime('-'.date("w").' day'.' +6 day'));
                $startDate = date("d.m", strtotime('-'.date("w").' day'.' +1 day'));
                $finishDate = date("d.m", strtotime('-'.date("w").' day'.' +6 day'));
            break;
        }
        ...
        $ret = array($day_n => array('subjects' => array($subject), 'themes' => array($theme), 'cabinets' => array($cabinet), 'homework' => array($homework), 'mark' => array($mark), 'date' => $d_date[$day]), 'weekdates' => $startDate.' - '.$finishDate);
